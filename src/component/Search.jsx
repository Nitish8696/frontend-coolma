...
const Search = () => {
    ...
    useEffect(() => {
        const handleSearchProducts = async () => {
            const response = await axios.get(`http://localhost:8000/api/search?q=${searchTerm}&page=${page}&limit=${limit}`)
                ...
                <div className='w-[90%] mx-auto mt-4 grid lg:grid-cols-2 gap-2 lg:mb-[50px]'>
                    {
                        products.map((product) => {
                            return <div key={product._id} className=' py-2 rounded'>
                                <Link to={`/product/${product._id}`}>
                                    <div className='flex justify-between flex-col gap-2'>
                                        <div className='flex gap-2'>
                                            <img src={`http://localhost:8000/${product.img[0]}`} alt="" className='object-cover w-[70px] h-[70px] rounded' />
                                            <div>
                                        ...
                                        <div className='flex items-end gap-2 justify-end'>
                                            ...
                                            <button className='p-2 text-[#00AFEF] text-xs font-medium rounded' style={{ border: "1.5px solid #00AFEF" }}>Add to Cart</button>
                                        </div>
                                    </div>
                                    <hr className='mt-2' />
                                </Link>
